...
public class AutoScanTest {
    ...
    @Test
    @DisplayName("DiscountPrice from DiscountPolicy")
    void getPrimaryDiscountPolicy() {
        ...
        Order qualifierServiceOrder = ac.getBean(OrderService2.class).createOrder(1L, "item1",20000);
    ...
    @Test
    @DisplayName("choosing belong several discount policies")
    void getAllDiscountPolicy() {
        Member testMember = new Member(1L, "tester", Grade.VIP);
        ac.getBean(MemberService.class).join(testMember);

        Order fixServiceOrder = ac.getBean(OrderService3.class).createOrder(1L,"item1", 20000, "fixDiscountPolicy");
        Order rateServiceOrder = ac.getBean(OrderService3.class).createOrder(1L,"item1", 20000, "rateDiscountPolicy");
